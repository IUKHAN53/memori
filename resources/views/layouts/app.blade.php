...
<nav
    class="mobile-nav fixed inset-x-0 top-0 z-50 flex items-center justify-center h-20 py-3 border-slate-100 dark:border-zink-500 navbar"
    id="navbar">
    <div class="mobile-nav-direction nav-direction container 2xl:max-w-[87.5rem] px-4 mx-auto flex items-center self-center w-full">
        <div class="shrink-0">
            <a href="{{route('home')}}">
                <img src="{{asset('logo-lg.png')}}" alt="" class="block h-6 dark:hidden" style="width:110px; height: auto;">
                <img src="{{asset('logo-lg.png')}}" alt="" class="hidden h-6 dark:block">
            </a>
        </div>
        <div class="mx-auto" style="margin-left: unset;">
            <ul id="navbar7"
                class="mobile-menu-list mobile-menu absolute inset-x-0 z-20 items-center hidden py-3 bg-white shadow-lg dark:bg-zink-600 dark:md:bg-transparent md:z-0 navbar-menu rounded-b-md md:shadow-none md:flex top-full ltr:ml-auto rtl:mr-auto md:relative md:bg-transparent md:rounded-none md:top-auto md:py-0">
                <li>
                        ...
                        <button type="button"
                            ...
                            <div class="rounded-full px-4">
                                <img src="{{auth()->user()->picture}}" alt=""
                                     class="w-[37.5px] h-[37.5px] rounded-full nav-pfp">
                            </div>
                        </button>
                        <div
                            ...
                            <h6 class="mb-2 font-normal text-slate-500 dark:text-zink-300">{{__('all.welcome_message')}}</h6>
                ...
                @else
                    <li class="menu-register-btn">
                        <a href="{{route('register')}}"
                           class="flex items-center justify-center w-full px-4 text-white btn solid-btn">
                            <span>{{ __('all.register') }}</span>
                        </a>
                    </li>
                    <li class="menu-login-btn">
                        <a href="{{route('login')}}"
                           class="flex items-center justify-center w-full px-4 text-white btn outline-btn">
                            <span>{{ __('all.login') }}</span>
                        </a>
                    </li>
                @endif
            </ul>
        ...
        <div class="flex gap-2">
            <div class="ltr:ml-auto rtl:mr-auto md:hidden navbar-toggale-button">
                <button type="button"
                        class="flex items-center  justify-center w-[37.5px] h-[37.5px] p-0 btn">
...
<button id="back-to-top"
        class="fixed flex items-center justify-center w-10 h-10 text-white bg-maincolor rounded-md bottom-10 right-10">
...
<script>
    ...
    window.addEventListener('livewire:load', function () {
            if (typeof lucide !== 'undefined') {
                lucide.createIcons(); // Reinitialize icons if necessary
            }
        });
</script>
@stack('scripts')
