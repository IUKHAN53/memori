    ...
    <div class="grid-cols-1 gap-0 lg:grid-cols-12">
        <div class="lg:col-span-5">
            <div class="card-body">
                ...
                @if($user)
                    <div class="max-w-sm bg-white overflow-hidden my-4" style="border: solid 1px #eeeeee; border-radius: 20px;">
                        <img class="w-full h-40 object-contain object-center" style="padding: 1rem 1rem 0 1rem;" src="{{$user->picture}}"
                             alt="avatar">
                        <div class="py-4 px-4">
                            ...
                            <div class="flex items flex-col center justify-center gap-2">
                                <!-- Instead of calling verifyAndAdd immediately, open modal to enter secret phrase -->
                                <a href="#" @click="modelOpen = true"
                ...
                @else
                    @if($wants_login)
                        <form wire:submit="login">
                            <div class="mb-3">
                                <x-input-label for="email" class="inline-block mb-2 text-base font-medium" :value="__('all.email')"/>
                                <x-text-input wire:model="form.email" type="email" required autofocus autocomplete="email" placeholder="{{ __('all.enter_email') }}"/>
                                <x-input-error :messages="$errors->get('email')"/>
                            </div>
                            <div class="mb-3">
                                <x-input-label for="password" class="inline-block mb-2 text-base font-medium" :value="__('all.password')"/>
                                <x-text-input wire:model="form.password" id="password" type="password" required placeholder="{{ __('all.enter_password') }}"/>
                                <x-input-error :messages="$errors->get('password')"/>
                            </div>
                            <div>
                                <div class="flex items-center gap-2">
                                    <input id="remember" wire:model="form.remember" class="w-4 h-4 border rounded-sm appearance-none bg-slate-100 border-slate-200 dark:bg-zink-600/50 dark:border-zink-500 checked:bg-custom-500 checked:border-custom-500 dark:checked:bg-custom-500 dark:checked:border-custom-500 checked:disabled:bg-custom-400 checked:disabled:border-custom-400" type="checkbox" value="">
                                    <label for="remember" class="inline-block text-base font-medium align-middle cursor-pointer">{{ __('all.remember_me') }}</label>
                                </div>
                            </div>
                            <div class="mt-10">
                            ...
                            <div class="mt-10 text-center">
                                <p class="mb-0 text-slate-500 dark:text-zink-200">{{ __('all.dont_have_account') }} <a href="#" wire:click="toggleLogin" class="font-semibold underline transition-all duration-150 ease-linear text-slate-500 dark:text-zink-200 hover:text-custom-500 dark:hover:text-custom-500">{{ __('all.sign_up') }}</a></p>
                            </div>
                        </form>
                    @else
                        <form wire:submit="register">
                            <div class="mb-3">
                                ...
                                <x-text-input wire:model="first_name" id="first_name" type="text" name="first_name" required autofocus autocomplete="first_name"/>
                                <x-input-error :messages="$errors->get('first_name')" class="mt-2"/>
                            </div>
                            <div class="mb-3">
                                <x-input-label for="last_name" :value="__('all.last_name')"/>
                                <x-text-input wire:model="last_name" id="last_name" type="text" name="last_name" required autofocus autocomplete="last_name"/>
                                <x-input-error :messages="$errors->get('last_name')" class="mt-2"/>
                            </div>

                            <div class="mb-3">
                                <x-input-label for="email" :value="__('all.email')"/>
                                <x-text-input wire:model="email" id="email" type="email" name="email" required autocomplete="email"/>
                                <x-input-error :messages="$errors->get('email')"/>
                            </div>
                            ...
                            <div class="mt-10 text-center">
                                <p class="mb-0 text-slate-500 dark:text-zink-200">{{ __('all.already_registered') }} <a href="#" wire:click="toggleLogin" class="font-semibold underline transition-all duration-150 ease-linear text-slate-500 dark:text-zink-200 hover:text-custom-500 dark:hover:text-custom-500">{{ __('all.login') }}</a></p>
                            </div>
                        </form>
                    @endif
                @endif
                ...
                <div x-cloak x-show="modelOpen"
                    ...
                    <form class="mt-5">
                        <div>
                            ...
                            <input placeholder="Secret Phrase" type="text" wire:model="secret_phrase"
