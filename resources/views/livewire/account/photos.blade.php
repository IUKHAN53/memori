    ...
    <x-modal name="add-photo-modal" :show="$errors->isNotEmpty()" focusable max>
        ...
        >
            <div wire:ignore class="tf-cropper-root">
                ...
                <!-- Cropping UI (no separate preview step) -->
                <div x-show="showCroppie" class="tf-cropper-modal-bg">
                    <div class="tf-cropper-modal">
                        <div>
                            <div class="m-auto" x-ref="croppie"></div>
                            <div class="flex justify-center items-center gap-2 mt-2">
                                <!-- Optional rotate button to manually adjust orientation -->
                                <button
                                    type="button"
                                    class="text-blue-600"
                                    x-on:click.prevent="croppie.rotate(90)"
                                >
                                    Rotate 90°
                                </button>
                                <button
            ...
            <!-- Single button to Crop and Upload -->
        ...
        Alpine.data('imageCropper', (config) => ({
            ...
            initCroppie() {
                this.croppie = new Croppie(this.$refs.croppie, {
                    enableExif: true,         // Enable EXIF orientation
                    enableOrientation: true,  // Allow rotation if needed
                    viewport: {
                        width: this.width,
                        height: this.height,
                        type: this.shape,
            ...
            // Crop the image and upload it in one step
            uploadPhoto() {
                this.croppie.result({
                    type: 'base64',
                    size: 'original'
                    ...
                    // Assign the cropped image to Livewire property
                    this.$wire.photo = croppedImage;
                    // Trigger Livewire event to save photo
