            ...
            <div class="flex flex-col">
                <div class=" flex  flex-row">
                    ...
                    <div>
                        ...
                        <p class="text-gray-500 mb-4">Lifetime: {{\Carbon\Carbon::parse($post->profile->date_of_birth)->format('d/m/Y')}}
                            - {{\Carbon\Carbon::parse($post->profile->date_of_death)->format('d/m/Y')}}
                            ({{$post->profile->age}} Yrs)</p>
                    </div>
                </div>
                <p class="text-gray-500 mb-4"><strong>Post Created at: </strong>{{$post->created_at->diffForHumans()}}</p>
                <p class="text-gray-500 mb-4"><strong>Post Title: </strong>{{$post->title}}</p>
                <p class="text-gray-500 mb-4"><strong>Post Content: </strong>{{$post->tribute}}</p>
