<x-app-layout>
    ...
    <section class="relative profile-container xl:py-32" id="">
        <div class="container 2xl:max-w-[87.5rem] mx-auto">
            <div>
                <div>
                    <img class="w-full h-58 banner-profile"
                         src="{{$profile->cover}}"
                         alt="">
                    <div class="under-banner">
                         <div class="flex justify-between items-center gap-3 flex-col md:flex-row my-5 profile-header">
                        <div class="flex justify-start items-center flex-col md:flex-row gap-3 profile-top">
                            ...
                            <div>
                                @if(!$profile->include_heading_text AND $profile->heading_text)
                                    <div
                                        class="text-md text-slate-500">{{$profile->heading_text ?? ''}}</div>
                                @endif
                                <div style="display: flex; align-items: center;">
    <h3 style="margin: 0; margin-left: 0.5rem;">{{$profile->full_name}}</h3>
    <img src="{{ asset('assets/images/flame.svg') }}" class="flame-icon" alt="Flame Icon" style="width: 0.8rem;">
                                ...
                                <div>
                                    @if($profile->is_public)
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-200 text-green-600 priv-profile">
                                            {{__('all.public_profile')}}
                                        </span>
                                    @else
                                        <span
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-200 text-red-600 priv-profile">
                        ...
                        <div class="flex justify-end items-end">
                            <div
                                ...
                                @if(auth()->user() && auth()->user()->id == $profile->user_id)
                                    ...
                                    @else
                                        <a href="{{route('change-status',['id' => $profile->id])}}"
                                           class="outline-btn btn text-white text-xs rounded shadow flex items-center gap-2">
                                ...
                                <button data-modal-target="shareModal"
                                        class="solid-btn-grey btn text-white text-xs rounded shadow flex items-center gap-2">
                        ...
                        <ul class="flex flex-wrap justify-center w-full text-sm font-medium text-center border-b border-slate-200 dark:border-zink-500 nav-tabs">
                            <li class="group active">
                                <a href="javascript:void(0);" data-tab-toggle data-target="photos"
                                   class="inline-block px-2 py-2 text-base transition-all duration-300 ease-linear rounded-t-md text-slate-400 dark:text-zink-200 border-b border-transparent group-[.active]:text-custom-500 group-[.active]:border-b-custom-500 hover:text-custom-500 active:text-custom-500 dark:hover:text-custom-500 dark:active:text-custom-500 dark:group-[.active]:hover:text-custom-500 -mb-[1px]">
                                    {{__('all.photos')}}
                                </a>
                            </li>
                            <li class="group">
                            ...
                            <div class="hidden tab-pane" id="details">
                                <div class="flex flex-col gap-4">
                                    <div class="flex flex-col md:flex-row gap-1">
                                        <h6 style="font-size: 1rem;">מגורים</h6>
                                        <p>{{$profile->city}}, {{$profile->state}}</p>
                                    </div>
                                    <div class="flex flex-col md:flex-row gap-1">
                                        <h6 style="font-size: 1rem;">{{__('all.cemetery_information')}}</h6>
                                        <p>{{$profile->cemetery_name}}, חלקה {{$profile->cemetery_plot}}</p>
                                        <p>{{$profile->cemetery_city}}, {{$profile->cemetery_state}}</p>
                                    </div>
